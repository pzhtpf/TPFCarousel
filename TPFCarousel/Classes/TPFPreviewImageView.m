...
- (void)restoreZoom {
    ...
    if (_zoomChanged) _zoomChanged(self, self.scrollView.zoomScale, self.imageView.center);
}
...
- (void)scrollViewDidZoom:(UIScrollView *)scrollView {
    ...
    if (_zoomChanged) _zoomChanged(self, self.scrollView.zoomScale, subView.center);
}
...
- (void)cancelLoading {
    ...
    if (self.loadingFinishedBlock) self.loadingFinishedBlock(self.loadingFinished);
}
...
//            self.url = _imageObject;
        ...
        self.loadingFinished = true;
        if (_loadingFinishedBlock) self.loadingFinishedBlock(self.loadingFinished);
    }
}
...
//    url = [NSString stringWithFormat:@"%@?x-oss-process=image/resize,w_1125/quality,Q_90",url];
    ...
    [self.imageView yy_setImageWithURL:[NSURL URLWithString:imgURL] placeholder:self.placeholderImage options:YYWebImageOptionSetImageWithFadeAnimation progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        ...
        } else weakSelf.loadingFinished = false;

        if (weakSelf.loadingFinishedBlock) weakSelf.loadingFinishedBlock(weakSelf.loadingFinished);
    }];
}
